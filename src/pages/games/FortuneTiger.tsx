...
const FortuneTiger: React.FC = () => {
  ...
  const checkWin = (finalReels: string[][]): number => {
    ...
    for (const combo of WINNING_COMBOS) {
      if (middleRow.every((s, i) => s === combo.symbols[i])) return combo.multiplier;
    }
    if (middleRow[0] === middleRow[1] && middleRow[1] === middleRow[2]) return 5;
  ...
  const spin = async () => {
    ...
    if (balanceError) { toast.error('Erro ao processar aposta'); setSpinning(false); return; }
    ...
    const spinInterval = setInterval(() => {
      ...
      if (spinCount >= 20) {
        ...
        if (multiplier > 0) {
          ...
          supabase.from('profiles').update({ balance: user.balance - bet + winAmount }).eq('id', user.id)
            .then(() => { updateBalance(winAmount); toast.success(`🐯 VITÓRIA! Ganhou € ${winAmount.toFixed(2)}!`); });
        }
        setSpinning(false);
      }
    }, 80);
      ...
      <div className="space-y-4 sm:space-y-6 max-w-2xl mx-auto">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">O tigre da fortuna</p>
          </div>
        </div>

        <div className="flex justify-center">
          <div className={`text-8xl sm:text-9xl transition-transform duration-500 ${tigerAnimation ? 'scale-110' : 'scale-100'}`}>🐯</div>
        </div>

        <Card className="bet-card overflow-hidden">
          <CardContent className="p-4 sm:p-6">
            <div className="bg-gradient-to-br from-primary/20 to-primary/5 rounded-lg p-4 border-2 border-primary/30">
              <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-4">
                {reels.map((reel, reelIndex) => (
                  <div key={reelIndex} className="bg-card rounded-lg p-2 border border-border">
                    {reel.map((symbol, symbolIndex) => (
                      <div key={symbolIndex} className={`text-4xl sm:text-5xl text-center py-2 ${symbolIndex === 1 ? 'bg-primary/10 rounded' : ''} ${spinning ? 'animate-pulse' : ''}`}>
                        {symbol}
                      </div>
                    ))}
                  </div>
              ...
              {lastWin !== null && (
                <div className="text-center mb-4 p-3 bg-primary/10 rounded-lg border border-primary/30">
                  <p className="text-sm text-primary">ÚLTIMO GANHO</p>
            ...
            <div className="mt-6 space-y-4">
              <div className="flex items-center gap-4">
                <div className="flex-1">
                  ...
                  <Input type="number" value={betAmount} onChange={(e) => setBetAmount(e.target.value)} min="0.10" step="0.10" disabled={spinning} className="mt-1" />
                </div>
              ...
              <Button onClick={spin} disabled={spinning || !user} className="w-full h-14 text-lg font-bold">
                {spinning ? (<><RotateCcw className="h-6 w-6 animate-spin mr-2" />GIRANDO...</>) : (<><Play className="h-6 w-6 mr-2" />GIRAR</>)}
              </Button>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              {WINNING_COMBOS.map((combo, i) => (
                <div key={i} className="flex items-center justify-between p-2 bg-muted rounded">
                  ...
                  <span className="font-bold text-primary">{combo.multiplier}x</span>
                </div>
              ))}
            </div>
          </CardContent>

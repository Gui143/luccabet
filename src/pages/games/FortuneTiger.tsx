    ...
    <div className="min-h-screen bg-[#0f0f15] text-white flex flex-col">
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between px-3 py-2.5 sm:px-6 sm:py-3 border-b border-white/5 bg-[#1a1a2e]/95 backdrop-blur-md">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-lg sm:text-xl font-black tracking-tight text-red-500">
            Bet
          </span>
        </div>
      ...
      <div className="flex-1 flex flex-col items-center px-0 sm:px-4 pb-0 sm:pb-4">
        ...
        <div
          id="game-container"
          className="relative w-full sm:max-w-5xl sm:mx-auto sm:rounded-xl overflow-hidden shadow-2xl border border-white/5 flex-1 sm:flex-none sm:aspect-video bg-[#000]"
        >
          {/* Loading overlay */}
          {loading && (
          ...
          <iframe
            src={GAME_URL}
            className={`w-full h-full absolute inset-0 transition-opacity duration-700 ${loading ? 'opacity-0' : 'opacity-100'} brightness-105 contrast-105`}
            frameBorder="0"
        ...
        <div className="w-full sm:max-w-5xl sm:mx-auto flex items-center justify-between gap-3 px-4 py-3 sm:mt-3 border-t sm:border-t-0 border-white/5 bg-[#0f0f15] sm:bg-transparent">
          <div className="flex items-center gap-2">
            <Button
              ...
              className="gap-1.5 text-xs bg-white/5 border-white/10 hover:bg-white/10 text-white"
            >
            ...
            <Button
              ...
              className={`gap-1.5 text-xs ${favorited ? 'bg-red-600 hover:bg-red-700' : 'bg-white/5 border-white/10 hover:bg-white/10 text-white'}`}
            >

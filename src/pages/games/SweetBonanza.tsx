...
import { ArrowLeft, Maximize, Heart, Wallet, Loader2, Candy } from 'lucide-react';
import { Link } from 'react-router-dom';
import { Button } from '@/components/ui/button';
import { useAuth } from '@/contexts/AuthContext';
import { formatBRLShort } from '@/lib/formatCurrency';

const GAME_URL = 'https://demogamesfree.pragmaticplay.net/gs2c/openGame.do?gameSymbol=vs20fruitswf&lang=pt&cur=BRL';
      ...
      <div className="flex-1 flex flex-col items-center px-0 sm:px-4 pb-0 sm:pb-4">
        <div className="hidden sm:flex items-center gap-3 w-full max-w-5xl mt-4 mb-3">
          <div className="flex-1">
            <h1 className="text-xl font-bold">Sweet Bonanza</h1>
            <p className="text-xs text-white/50">Pragmatic Play • Ganhe até 21.175x</p>
          </div>
        </div>
          ...
          {loading && (
            <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-[#0a0a0f] gap-4">
              <div className="relative">
                <div className="w-16 h-16 rounded-full border-4 border-white/5 border-t-pink-500 animate-spin" />
                <Candy className="absolute inset-0 m-auto h-6 w-6 text-pink-500 animate-pulse" />
              </div>
              <div className="text-center">
                <p className="text-sm font-semibold text-white">Carregando Sweet Bonanza...</p>
        ...
        <div className="w-full sm:max-w-5xl sm:mx-auto flex items-center justify-between gap-3 px-4 py-3 sm:mt-3 border-t sm:border-t-0 border-white/5 bg-[#0f0f15] sm:bg-transparent">
          <div className="flex items-center gap-2">
            ...
            <Button variant={favorited ? 'default' : 'outline'} size="sm" onClick={() => setFavorited(!favorited)} className={`gap-1.5 text-xs ${favorited ? 'bg-pink-600 hover:bg-pink-700' : 'bg-white/5 border-white/10 hover:bg-white/10 text-white'}`}>
